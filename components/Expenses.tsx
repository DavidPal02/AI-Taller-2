...
export const Expenses: React.FC = () => {
  ...
  const loadData = async () => {
    ...
    // Set default category for new expense if exists
  ...
  const handleCategoryChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
      ...
      setNewExpense({
          ...newExpense,
          category: catName,
          amount: cat?.defaultAmount || newExpense.amount // Auto-fill amount if fixed
      });
  };
  ...
  // Calculate Breakdown for Report
  ...
  return (
    <div className="p-8 h-full overflow-hidden flex flex-col">
            ...
            filteredExpenses.map((expense) => (
              ...
              >
                ...
                <div className="col-span-2 text-slate-500 text-sm italic">
                  {expense.jobId ? `Trabajo #${expense.jobId.slice(0,4).toUpperCase()}` : 'General'}
                </div>
      ...
      {/* Monthly Report Modal (Printable Sheet) */}
      <AnimatePresence>
         {showReport && (
               ...
               >
                  ...
                  <div className="mb-10 border-b-2 border-slate-100 pb-6">
                      ...
                      <div className="flex justify-between items-end mt-4">
                          ...
                          <div className="text-right">
                              <h2 className="text-lg font-bold text-slate-800">Taller Peter</h2>
                              <p className="text-sm text-slate-500">Av. Principal 123, Ciudad</p>
                          </div>
                      </div>
                  </div>
                  ...
                  <div className="mb-12">
                      ...
                      <div className="grid grid-cols-4 gap-4">
                          {/* Fix: Explicitly cast Object.entries to ensure 'amount' is treated as a number */}
                          {(Object.entries(categoryBreakdown) as [string, number][]).map(([category, amount]) => (
                              <div key={category} className="bg-slate-50 p-4 rounded-lg border border-slate-100">
                                  ...
                                  {/* Fix: Correct toLocaleString usage by ensuring 'amount' is recognized as a number */}
                                  <p className="text-xl font-bold text-slate-800">{amount.toLocaleString('es-ES', { minimumFractionDigits: 2 })} €</p>
                                  {/* Fix: Ensure 'amount' and 'totalExpenses' are numbers for arithmetic operations and handle division by zero */}
                      ...
                      <table className="w-full text-sm text-left">
                              ...
                              {filteredExpenses.map((expense) => (
                                  <tr key={expense.id} className="hover:bg-slate-50">
                                      ...
                                      <td className="p-3 text-slate-400 italic">
                                          {expense.jobId ? `#${expense.jobId.slice(0,4).toUpperCase()}` : '-'}
                                      </td>
                          ...
                          <tfoot>
                              <tr className="border-t-2 border-slate-800">
                                  <td colSpan={4} className="p-4 text-right font-bold text-slate-800 text-lg">TOTAL MENSUAL</td>
                                  <td className="p-4 text-right font-bold text-slate-900 text-xl">{totalExpenses.toLocaleString('es-ES', { minimumFractionDigits: 2 })} €</td>
                              </tr>
                          </tfoot>
                      </table>
                  </div>
                  ...
                  <div className="mt-12 text-center text-slate-400 text-xs pt-8 border-t border-slate-100">
                      <p>Documento generado internamente por Taller Manager Pro.</p>
                  </div>
               </motion.div>
            </div>
         )}
